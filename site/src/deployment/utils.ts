...
import { SpawnOptions, SubmoduleEntry } from 'git-command-helper';
 ...
 *
 * @param submodule - Submodule metadata (contains `path`, `url`, `branch`).
 * @param rootProjectPath - Filesystem path to the repository that contains the submodule.
 * @throws Will throw or call `process.exit(1)` if git operations ultimately fail.
 */
export function resetSubmodule(submodule: SubmoduleEntry, rootProjectPath: string) {
