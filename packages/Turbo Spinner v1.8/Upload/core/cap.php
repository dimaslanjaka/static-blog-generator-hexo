...
function elite_captcha($color, $mode, $mul, $allowed)
{
    ...
    if ($mul == "on")
    {
        $captcha_config = array(
            ...
            'backgrounds' => array(
                ...
                $bg_path . 'text2.png',
                $bg_path . 'text1.png'),
            'fonts' => array($font_path . $fonr_name),
            'characters' => $allowed,
            'min_font_size' => 28,
            'max_font_size' => 28,
            'color' => $color,
    ...
    } else
    {
        $captcha_config = array(
            ...
            'min_length' => 5,
            'max_length' => 5,
            'backgrounds' => array($bg_path . 'text2.png'),
 ...
 ///   }
    ...
    if (empty($captcha_config['code']))
    {
        ...
        {
            $captcha_config['code'] .= substr($captcha_config['characters'], rand() % (strlen
                ($captcha_config['characters'])), 1);
        }
    }
...
if (isset($_GET['_CAPTCHA']))
{
    ...
    $angle = rand($captcha_config['angle_min'], $captcha_config['angle_max']) * (rand
        (0, 1) == 1 ? -1 : 1);
    ...
    if ($captcha_config['shadow'])
    {
        ...
        $shadow_color = imagecolorallocate($captcha, $shadow_color['r'], $shadow_color['g'],
            $shadow_color['b']);
    ...
    // Draw text
    imagettftext($captcha, $font_size, $angle, $text_pos_x, $text_pos_y, $color, $font,
        $captcha_config['code']);
